...
Download and install the latest release package:
    ...
    curl -L -O https://github.com/limserhane/sgcsfuse/releases/download/v0.34.1/gcsfuse-0.34.1-1.x86_64.rpm
...
If you are on a distribution that uses `.rpm` files for package management:
    ...
    curl -L -O https://github.com/limserhane/sgcsfuse/releases/download/v0.32.0/gcsfuse-0.32.0-1.x86_64.rpm
    sudo rpm --install -p gcsfuse-0.32.0-1.x86_64.rpm

Or one that uses `.deb` files:

    curl -L -O https://github.com/limserhane/sgcsfuse/releases/download/v0.32.0/gcsfuse_0.32.0_amd64.deb
...
To install or update gcsfuse, run:
    ...
    GO111MODULE=auto go get -u github.com/limserhane/sgcsfuse

This will fetch the gcsfuse sources to
`$GOPATH/src/github.com/limserhane/sgcsfuse`, build them, and install a
